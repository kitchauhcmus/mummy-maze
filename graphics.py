...
# CLASS XỬ LÝ CỔNG (GATE)
class gate_spritesheet:
    def __init__(self, image_spritesheet_path):
        self.sheet = pygame.image.load(image_spritesheet_path)
        self.rect = self.sheet.get_rect()
        w = self.rect.width / 8    # 8 là số sheet trong ảnh
        h = self.rect.height
        self.cells = []
        for x in range(8):
...
class stairs_spritesheet:
    def __init__(self, image_spritesheet_path):
        ...
        # up, right, down, left = (0, 1, 3, 4) trong list stairs
        self.stairs = []
        for x in range(4):
...
# HÀM VẼ MÀN HÌNH CHÍNH CỦA GAMEPLAY
def draw_screen(screen, input_maze, backdrop, floor, maze_size, cell_rect, stair, stair_position, trap, trap_position,
    ...
    # --- VẼ TRAP ---
    if trap_position:
        for t_pos in trap_position: # Sửa lỗi IndexError bằng vòng lặp
            trap_x = coordinate_X + cell_rect * (t_pos[1] // 2)
            trap_y = coordinate_Y + cell_rect * (t_pos[0] // 2)
...
def determine_moving_direction(past_position, new_position):
    if past_position[0] == new_position[0] + 2:  # Đi lên
        return "UP"
    if past_position[0] == new_position[0] - 2:  # Đi xuống
        return "DOWN"
    if past_position[1] == new_position[1] + 2:  # Qua trái
        return "LEFT"
    if past_position[1] == new_position[1] - 2:  # Qua phải
        return "RIGHT"

# ANIMATION KẺ ĐỊCH DI CHUYỂN
